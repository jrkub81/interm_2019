...
<body>
    ...
    <?php 
   include 'condb.php';
   //------Post data and send update to db---------
...
<hr>
    <table class = my-table>
    <h2 style = "text-align: center">แก้ไขรูปภาพรถยนต์</h2>
    <tr>
    ...
    if (!empty($_GET["id"])) {
        ...
        $result1 = $conn->query($sql_brand); 
    while($row2 = $result1->fetch_assoc()){
        if ($row["in_carsbrand"]==$row2["cb_id"]) {
    ?>
    <td><?=$row2["cb_name"]?></td>
    <?php 
    }
    ...
    <input type= "hidden" name= "up_carsid" value ="<?=$row["in_carsid"]?>" >
    </table>
    <br>
    <button class = button_3  type="submit" name="upfile">
    ส่งข้อมูล
    </button>
    </form>
    <?php
